...
var AnswersCurtain = [
    //Default Value 
    {"City" :'Dubai'},
    {'Do you know the size of the curtains to be cleaned?' :'Yes'},
    {'Number of curtain you want to get Curtain?' : '1'},
    {"Tell us the size(s) of your Curtain(s):" : {}},
...
$('.inner .type span').click(function(){
    ...
    AnswersCurtain[1] ={'Do you know the size of the Curtain to be cleaned?' :$(this).text()}
});
$('.inner .NRooms span').click(function(){
    $(this).addClass('active').siblings().removeClass('active')
    AnswersCurtain[2] ={'Number of Curtain you want to get cleaned?'  :$(this).text()}
});
...
$('.inner .msg textarea').change(function(){
    AnswersCurtain[4] ={"Do you have any specific instructions?" : $(this).val()}
});
...
$('.Details .inner .Next:eq(0)').click(function(){
    // validation for City 
    if($('.city').data('class')){
        AnswersCurtain[0] = {"City" : $('.city').data('class')};
        ...
        if($('.inner .Size-Type input:eq(0)').val() &&  $('.inner .Size-Type input:eq(1)').val()){
            ...
            if($('.inner .Size-Type input:eq(0)').val() > 0 &&  $('.inner .Size-Type input:eq(1)').val() > 0){
                ...
                AnswersCurtain[3] ={'Tell us the size(s) of your carpet(s)' : {'X' :`${$('.inner .Size-Type input:eq(0)').val()} ${$('.inner .Size-Type .unit').text()}` , 'Y':`${$('.inner .Size-Type input:eq(1)').val()} ${$('.inner .Size-Type .unit').text()}`}}
...
$('#MapContinue').click(function(){
    //check input filed 
    if($('.DetailsBlock input:eq(1)').val().length && $('.DetailsBlock input:eq(2)').val()){
        $('.DetailsBlock span').fadeOut();
        // $(this).attr('disabled' , false);
        AnswersCurtain[4] = {'Address' : $('.DetailsBlock input:eq(0)').val()}
